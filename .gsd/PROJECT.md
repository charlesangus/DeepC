...
**v1.2 + DeepCBlur v2 + DeepCDepthBlur correctness + DeepCOpenDefocus shipped.** 25 plugins. DeepCOpenDefocus is a new GPU-accelerated deep defocus Nuke plugin built on the opendefocus 0.1.10 Rust engine (wgpu/Vulkan): accepts Deep image input, applies layer-peel algorithm with per-sample CoC from depth, outputs physically-based defocused flat RGBA. Features: holdout Deep input (∏(1−α) transmittance attenuation), CameraOp input link overriding lens knobs, 4 manual lens knobs (focal_length_mm, fstop, focus_distance, sensor_size_mm), all non-uniform bokeh artifacts (catseye, barndoor, astigmatism, axial aberration, inverse foreground), aperture-iris icon, EUPL-1.2 attribution. Docker-verified: 21 MB DeepCOpenDefocus.so (Nuke 16.0 Linux). All 13 requirements R046–R058 validated. M009 complete.
