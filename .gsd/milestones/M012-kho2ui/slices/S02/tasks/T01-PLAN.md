...
Add the full-image PlanarIop render cache to DeepCOpenDefocus, create the 256×256 Nuke test script, and extend the verify script to check for it.
...
- ``src/DeepCOpenDefocus.cpp` — current renderStripe + _validate implementation from S01`
- ``scripts/verify-s01-syntax.sh` — existing syntax + artifact check script`
- ``crates/opendefocus-deep/include/opendefocus_deep.h` — FFI signatures (quality param added in S01)`

## Expected Output

- ``src/DeepCOpenDefocus.cpp` — cache members, _cacheValid invalidation in _validate, full-image renderStripe with cache hit/miss logic`
- ``test/test_opendefocus_256.nk` — 256×256 single-layer Nuke test script for SLA proof`
- ``scripts/verify-s01-syntax.sh` — extended to check for test_opendefocus_256.nk`

## Verification

bash scripts/verify-s01-syntax.sh && grep -q '_cacheValid' src/DeepCOpenDefocus.cpp && grep -q '_cacheMutex' src/DeepCOpenDefocus.cpp && grep -q 'info_.box()' src/DeepCOpenDefocus.cpp && test $(grep -c 'Enumeration_knob' src/DeepCOpenDefocus.cpp) -eq 1 && test -f test/test_opendefocus_256.nk
