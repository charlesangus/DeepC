...
class DeepCGW : public DeepCWrapper
{
    ...
    public:
        ...
        DeepCGW(Node* node) : DeepCWrapper(node)
        {
            for (int i=0; i<3; i++)
            {
                blackpoint[i] = 0.0f;
...
void DeepCGW::_validate(bool for_real)
{
...
*/
void DeepCGW::wrappedPerSample(Box::iterator it, DeepPixel deepInPixel, size_t sampleNo, float &perSampleData)
{
    perSampleData = 1.0f;
}
...
*/
void DeepCGW::wrappedPerChannel(const float inputVal, float perSampleData, Channel z, float& outData)
{
...
void DeepCGW::custom_knobs(Knob_Callback f)
{
...
const char* DeepCGW::node_help() const
{
    return
...
static Op* build(Node* node) { return new DeepCGW(node); }
const Op::Description DeepCGW::d("DeepCGW", 0, build);
