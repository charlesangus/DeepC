...
{
    DEEPC_RAW_DEFOCUS_MODE = 0,
    DEEPC_MODIFIED_DEFOCUS_MODE,
...
struct DeepCSpec
{
    ...
    std::vector<std::vector<float>> volumetricFalloffOffsets;
    bool volumetricBlur;

    std::vector<float> computeVolumetrics(const int blurRadius) const;
};